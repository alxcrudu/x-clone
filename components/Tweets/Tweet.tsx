...
const Tweet: React.FC<ITweetProps> = ({ tweet, session, isForTweetPage }) => {
  ...
  const { mutate: mutateTweets } = useInfiniteTweets(session.user.id);
  const { data: fetchedTweet, mutate: mutateTweet } = useTweet(tweet.id);

  if (isForTweetPage) {
    return (
      <div className="w-full border-b-2 border-double border-neutral-800 p-1.5">
        <div className="flex w-full flex-col gap-4 px-3 py-2">
          <div className="flex gap-4">
            ...
            <div className="flex flex-col">
              ...
              {tweet.user.verified && tweet.user.verified === true ? (
                <div>
                  <Verified />
                </div>
              ) : null}
  ...
  return (
    <>
      <div
        onClick={goToTweet}
        className="flex w-full cursor-pointer border-b-2 border-double border-neutral-800 p-1.5 duration-100 hover:bg-slate-400 hover:bg-opacity-5"
      >
